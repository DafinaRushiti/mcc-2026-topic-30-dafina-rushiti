...
const OrderRow = ({ order, onUpdateStatus, onStatusChange, onDeleteOrder }) => {
  ...
  const handleDelete = async () => {
    const confirmed = window.confirm(`Delete order #${safeOrder.id}?`);
    if (!confirmed) return;

    setDeleting(true);
    try {
      await onDeleteOrder(safeOrder.id);
    } catch (error) {
...
const OnlineOrdersPanel = () => {
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      ...
      console.error('Error deleting order:', error);
      const errorMsg = error.response?.data?.msg || error.message || 'Unknown error occurred';
      setError(`Failed to delete order: ${errorMsg}`);
      setShowSuccessMessage(false);
      return false;
    }
  };
