...
exports.getDailyReports = async (req, res) => {
  try {
    ...
    orders.forEach(order => {
      if (!reportsByUser[order.userId]) return;

      const orderItems = order.OrderDetails || [];
      const itemCount = orderItems.reduce(
        (sum, detail) => sum + (parseInt(detail.quantity) || 0),
        0
      );
      const totalPrice = parseFloat(order.totalPrice || 0);
      const report = reportsByUser[order.userId];
    ...
    const formattedReports = Object.values(reportsByUser).map(report => ({
      ...report,
      details: {
        ...report.details,
