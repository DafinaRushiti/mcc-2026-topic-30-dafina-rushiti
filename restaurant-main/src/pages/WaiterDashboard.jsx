...
export default function WaiterDashboard() {
  ...
  const getActiveOrderForTable = (table) => {
    return activeOrders.find(order => parseInt(order.tableNumber) === table);
  };

  const clearTableState = (table) => {
    setOrders(prev => {
    ...
    setActiveOrders(prev => prev.filter(order => parseInt(order.tableNumber) !== table));
  ...
  const completePlacedOrderForTable = async (table) => {
    if (!orderPlacedMap[table]) return;

    const activeOrder = getActiveOrderForTable(table);
    let orderId = selectedTable === table && currentOrderId
      ? currentOrderId
      : activeOrder?.id;
  ...
  const handleClearTableFromMenu = async (table) => {
    setLoading(true);
    setError("");
    try {
      await completePlacedOrderForTable(table);
      clearTableState(table);
      ...
      setError(err.response?.data?.msg || "Failed to clear table");
      setSnackbarOpen(true);
    } finally {
      setLoading(false);
    }
  ...
  const clearTable = async () => {
    ...
    setError("");
    try {
      await completePlacedOrderForTable(selectedTable);
      clearTableState(selectedTable);
      handleClose();
